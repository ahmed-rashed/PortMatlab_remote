...
CALL mxCopyComplex16ToPtr(EigVectors_Normalized, mxGetPr(PLHS(1)), mxGetPi(PLHS(1)), N*2*N)

IF (NLHS > 1) THEN
    PLHS(2)=mxCreateDoubleMatrix(2*N,1,1)  ! Create the output matrix
CALL mxCopyComplex16ToPtr(EigValues_vec, mxGetPr(PLHS(2)), mxGetPi(PLHS(2)), 2*N)
END IF
